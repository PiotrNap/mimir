...
A vesting script is a smart contract that locks up funds and allows the beneficiary to withdraw them after the lockup period.
There are many use cases for a vesting script:
1. DAO treasury locks - DAO funds can be vested linearly and only be withdrawn by the beneficiary.
2. Employee/Contributor payment - when a new employee joins an organization, they typically receive a promise of compensation to be disbursed after a specified duration of employment.
3. Token vesting - projects that raise funds often want to lock their own tokens so founders can't dump them immediately.
4. Escrow/Legal Settlement - involves two parties transacting with each other, potentially bound by legal agreement and a mediator.

In this guide, we will walk through examples 1. and 2.

## DAO Treasury Locks

This script is designed for a linear distribution of a DAO's treasury funds, with pre-defined conditions that need to be met for the transaction to be successful.
Depending on the initiated action, one can either withdraw funds, update the vesting schedule, pause the lock-up period, or update the beneficiary's information.
This script gives both flexibility for authorized actors to change the vesting parameters, and the security to ensure that only the vested amount can be collected by the rightful recipient.

### On-Chain Code



For the sake of simplicity, we will cut out all helper functions and leave only the logic responsible for validating the transaction.
...
## Employee/Contributor Payment

This arrangement often involves the organization depositing the funds into a vesting contract,
with the employee gaining access to the funds upon the completion of a predetermined lockup period.
Through the utilization of vesting contracts, organizations establish a mechanism to encourage employee retention by linking financial rewards to tenure.
...
This datum can be found [here](https://github.com/MeshJS/examples/blob/main/aiken-vesting/aiken-workspace/validators/vesting.ak#L8).
...
This validator can be found [here](https://github.com/MeshJS/examples/blob/main/aiken-vesting/aiken-workspace/validators/vesting.ak#L17).
...
We recommend you to check out [this code](https://github.com/MeshJS/examples/blob/main/aiken-vesting/aiken-workspace/validators/vesting.ak) to learn more.
...
This command will generate a CIP-0057 Plutus blueprint, which you can review [here](https://github.com/MeshJS/examples/blob/main/aiken-vesting/aiken-workspace/plutus.json).
...
To execute this code, ensure you have defined blockfrost key in the `.env` file (view an example file [here](https://github.com/MeshJS/examples/blob/main/aiken-vesting/.env.example)).
...
To execute this code, copy the transaction hash from the deposit transaction (you will need it when prompted during the next step). Ensure you have defined the blockfrost key in the `.env` file, as well as your wallet mnemonic.
